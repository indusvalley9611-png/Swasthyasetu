...
export function NewReplenishmentRequestModal({
  ...
  // Clean form reset whenever modal opens
  React.useEffect(() => {
    if (isOpen) {
      const facId = user?.facilityId || 'fac-phc-velhe';
      setSelectedFacilityId(facId);
      const facStock = stocks.filter(s => s.facilityId === facId);
      setRows([
        {
          id: `row-init-${Date.now()}`,
          drugName: facStock[0]?.drugName || 'Anti-Snake Venom (ASV Polyvalent Lyophilized)',
          quantity: 10,
          unit: facStock[0]?.unit || 'Vials',
          urgency: 'CRITICAL',
          reason: 'Buffer replenishment requisition',
          selectedStockId: facStock[0]?.id,
        },
      ]);
      setRequestNotes('Facility medicine replenishment order');
      setOverallUrgency('CRITICAL');
      setErrorMessage('');
    }
  }, [isOpen, user?.facilityId, stocks]);
